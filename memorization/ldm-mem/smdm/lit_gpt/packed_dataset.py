...
class PackedDatasetIterator:
    ...
    def __next__(self):
        ...
        # ✅ 先算出 src_filename（放到 try 之前，这样出错也能打印）
        current_file_index = self._file_idx - self._n_chunks + chunk_id
        if 0 <= current_file_index < len(self._filenames):
            src_filename = self._filenames[current_file_index]
        else:
        ...
        block_id = int(block_idx % self._n_blocks)
        self._curr_idx += 1
        return torch.from_numpy(arr.astype(np.int64)), src_filename, block_id
