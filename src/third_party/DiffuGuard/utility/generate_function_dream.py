...
# Default Dream mask token id used by upstream DiffuGuard.
DEFAULT_MASK_ID = 151666

# ------------------ Default logits hook plumbing ------------------
...
def resolve_logits_hook(
    ...
    *,
    ...
    if logits_hook is None and _DEFAULT_HOOK_FACTORY is not None:
        ctx = dict(_DEFAULT_LOGITS_HOOK_CTX or {})
        if attention_mask is not None:
            ctx["attention_mask"] = attention_mask
        if mask_token_id is not None:
            ctx["mask_token_id"] = int(mask_token_id)
        if prompt_width is not None:
            ctx["prompt_width"] = int(prompt_width)
        if total_steps is not None:
            ctx["total_steps"] = int(total_steps)
        if vocab_size is not None:
            ctx["vocab_size"] = int(vocab_size)
        if embed_fn is not None:
            ctx["embed_fn"] = embed_fn
        return _DEFAULT_HOOK_FACTORY(tokenizer, device, **ctx)
    return logits_hook
...
def generate_dream_hidden(
    *,
    ...
    attack_method_lower = (attack_method or "").lower()
    controller_alpha0 = 0.0 if attack_method_lower in {"", "none"} else alpha0
    controller = AdaptiveRemaskController(
        mask_id=int(mask_id),
        block_start=int(block_start),
        block_end=int(block_end),
        steps_per_block=int(steps_per_block),
        alpha0=float(controller_alpha0),
    ...
    if logits_hook is None:
        hook_ctx = dict(logits_hook_ctx or {})
        hook_ctx.setdefault("attention_mask", cur_attn)
        hook_ctx.setdefault("prompt_width", input_len)
        hook_ctx.setdefault("total_steps", int(steps_per_block))
        hook_ctx.setdefault("vocab_size", getattr(getattr(model, "config", None), "vocab_size", None))
        hook_ctx.setdefault("embed_fn", None)
        logits_hook = resolve_logits_hook(tokenizer, device, **hook_ctx)

    def _compose_logits_hook(step: int, x: torch.Tensor, logits: torch.Tensor) -> torch.Tensor:
        logits = controller.logits_hook(step, x, logits)
    ...
    try:
        out = model.diffusion_generate(
            ...
            steps=int(steps_per_block),
            max_new_tokens=int(planned_new_tokens) if planned_new_tokens > 0 else None,
