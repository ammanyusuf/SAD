...
class PPL_calculator:
    def __init__(self, model=None, device = "cuda", max_length=None,  stride=None):
        default_model = os.getenv("PPL_GPT2_MODEL", "gpt2")
        self.model_id = os.path.expanduser(model or default_model)
        assert 'gpt2' in self.model_id, "Only GPT-2 models are supported"
        self.device = device
...
class Defender:
    ...
    def __init__(self, defense=None, retoken_dropout_rate=0.2, merge_table_path='/mnt/petrelfs/wenzichen/diffusion_lm/SATA/src/subword_nmt.voc', para_model='gpt-4o') -> None: # 'gpt-3.5-turbo'
        ...
        elif defense in {'diffuguard', 'diffguard'}:
            # DiffuGuard safety is applied inside the sampling loop (hidden audit + repair),
            # so we do not rewrite the prompt here. Treat this as a no-op defense.
    ...
    def no_defense(self, instruction):
        return instruction
