...
def _resolve_local_model_path(model_id: str) -> str:
    ...
    if cache_root:
        cache_candidates = [
            pathlib.Path(cache_root) / model_id,
            pathlib.Path(cache_root) / pathlib.Path(model_id).name,
        ]
    ...
    if hf_home:
        ...
        snapshots_dir = hub_dir / key / "snapshots"
...
def strongreject_finetuned_small_batch(
    ...
    evaluator_path: str = "~/DIJA/hf_models/strongreject-15k-v1",
    ...
    **kwargs,
    ...
    """
    if "strongreject_finetuned" not in cached_models:
        adapter_path = pathlib.Path(os.path.expanduser(evaluator_path))
        ...
        if os.getenv("TESTING"):
            ...
            try:
                ...
                adapter_meta = {}
            base_model = adapter_meta.get("base_model_name_or_path", evaluator_path)
            model_name = _resolve_local_model_path(str(base_model))
            adapter_name = str(adapter_path)
        else:
            model_name = evaluator_path
            adapter_name = None
        ...
        if adapter_name:
            try:
                ...
                raise RuntimeError(
                    "PEFT is required to load StrongREJECT adapter but is not installed."
                ) from exc
