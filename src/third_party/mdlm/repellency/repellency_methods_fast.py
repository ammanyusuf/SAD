"""Base repellency method registry and interface."""
...
class RepellencyMethod(ABC):
    def __init__(self, ref_data, embed_fn, forward_fn, num_timesteps, max_idx, beta_min, beta_max, n_embed, **kwargs):
        ...
        self.eta_from_scale = False
        eta = kwargs.get("eta", None)
        if eta is None and scale is not None:
            self.eta_from_scale = True
            eta = scale
        self.eta = float(eta) if eta is not None else 1.0
        self.weight_mode = kwargs.get("weight_mode", "eta_beta_hat")
        self.beta_hat_mode = kwargs.get("beta_hat_mode", "mc_mean")
        self.beta_hat_clip_min = kwargs.get("beta_hat_clip_min", None)
        self.beta_hat_clip_max = kwargs.get("beta_hat_clip_max", None)
        self.beta_hat_log_clip_min = kwargs.get("beta_hat_log_clip_min", -30.0)
        self.beta_hat_log_max_shift = kwargs.get("beta_hat_log_max_shift", 40.0)
        self.schedule_mode = kwargs.get("schedule_mode", "hard_window")
        self.epsilon = kwargs.get('epsilon', 1e-8)
        
        self.proj_ref_path = kwargs.get('proj_ref_path', None)
        self.cache_proj_ref = kwargs.get('cache_proj_ref', False)

        critical_steps = kwargs.get("critical_steps", None)
        if critical_steps is not None:
            self.critical_steps = {int(step) for step in critical_steps}
        else:
            self.critical_steps = None
        default_steps = num_timesteps if num_timesteps not in (None, 0) else max_idx
        default_t_end = max(default_steps - 1, 0) if default_steps is not None else 0
        self.t_start = kwargs.get("t_start", 0)
        if not self.t_start:
            self.t_start = 0
        self.t_end = kwargs.get("t_end", default_t_end)
        if not self.t_end:
            self.t_end = default_t_end
    ...
    def should_apply_at(self, t_idx: int | None) -> bool:
        if t_idx is None:
            return True
        if isinstance(t_idx, torch.Tensor):
            t_idx = int(t_idx.item())
        if self.critical_steps is not None:
            return t_idx in self.critical_steps
        # print(f"[Repellency] Applying from t={self.t_start} to t={self.t_end}, current t={t_idx}")
        return self.t_start <= t_idx <= self.t_end
   
    def conditioning(self, x_0_hat, **kwargs):
        t_idx = kwargs.get("t_idx", None)
        if os.environ.get("SAFE_REPELLENCY_DEBUG"):
            print(f"[Repellency] t_idx: {t_idx}")

        if t_idx is not None and not self.should_apply_at(t_idx):
