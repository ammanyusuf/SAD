...
class LLaDADiffuGuardBackend:
    name = "llada_diffuguard"
    family = "llada"
    ...
    def __init__(self) -> None:
        ...
        self._logged_mask_check: bool = False
    ...
    def generate_batch(
        ...
        if safety.enabled:
            unsafe_artifacts = safety.unsafe_artifacts
            unsafe_prototypes = safety.unsafe_prototypes
            artifacts_exist = bool(unsafe_artifacts and Path(unsafe_artifacts).exists())
            prototypes_exist = bool(unsafe_prototypes and Path(unsafe_prototypes).exists())
            msg = (
                "DiffuGuard safety enabled; unsafe_artifacts=%s (exists=%s) unsafe_prototypes=%s (exists=%s) "
                "unsafe_artifact_name=%s unsafe_artifact_root=%s use_semantic_gating=%s t_start=%s t_end=%s "
                "scale=%s eta=%s schedule=%s"
            )
            LOGGER.warning(
                msg,
                "DiffuGuard safety enabled; unsafe_artifacts=%s (exists=%s) unsafe_prototypes=%s (exists=%s) "
                "unsafe_artifact_name=%s unsafe_artifact_root=%s use_semantic_gating=%s t_start=%s t_end=%s "
                "scale=%s eta=%s schedule=%s",
                str(unsafe_artifacts) if unsafe_artifacts else None,
                artifacts_exist,
                str(unsafe_prototypes) if unsafe_prototypes else None,
                prototypes_exist,
                safety.unsafe_artifact_name,
                str(safety.unsafe_artifact_root) if safety.unsafe_artifact_root else None,
                safety.use_semantic_gating,
                safety.t_start,
                safety.t_end,
                safety.scale,
                safety.eta,
                safety.schedule_mode,
            )
            print(
                "[diffuguard] " + msg
                % (
            ...
            if logits_hook is None:
                LOGGER.warning(
                    "Safety enabled but safe denoiser hook could not be constructed (unsafe_artifacts=%s, "
                    "unsafe_prototypes=%s).",
                    str(unsafe_artifacts) if unsafe_artifacts else None,
                    str(unsafe_prototypes) if unsafe_prototypes else None,
                )
                print(
                    "[diffuguard] safe denoiser hook is None (unsafe_artifacts=%s, unsafe_prototypes=%s)."
                    % (
                        str(unsafe_artifacts) if unsafe_artifacts else None,
                        str(unsafe_prototypes) if unsafe_prototypes else None,
                    ),
        ...
        if generation.max_new_tokens % block_length != 0:
            LOGGER.warning(
                "DiffuGuard requires gen_length divisible by block_length; using block_length=gen_length."
            )
            block_length = generation.max_new_tokens
        ...
        if prompts:
            for batch in _chunk(prompts, generation.batch_size):
                ...
                if not self._logged_mask_check:
                    mask_id = int(self.mask_token_id or LLADA_MASK_TOKEN_ID)
                    mdm_token_id = None
                    try:
                        mdm_token_id = self.tokenizer.convert_tokens_to_ids("<|mdm_mask|>")
                    except Exception:
                        mdm_token_id = None
